...
function fuzzyMatch(query, text) {
  ...
  // Adjusted threshold to 50% of query length.
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function createStudyNote(study) {
    // Default study.sections to an empty array if undefined.
    ...
    fetch('studies.json')
      ...
      .then(data => {
        ...
        data.forEach(study => {
          ...
          // Create study note for "All Studies" container.
          const noteElemAll = createStudyNote(study);
          studyContainer.appendChild(noteElemAll);
          // Also, if recommended, add a separate study note in the "Recommended Studies" container.
          if (study.recommended === true) {
  ...
  // Search functionality: when searching, only look in the All Studies container to avoid duplicates.
  document.getElementById('search-bar').addEventListener('input', function() {
    collapseAllStudies(); // Collapse any open study notes.
    const query = this.value.toLowerCase();
    let notes;
    if (query === "") {
      // When no query, search all study notes (duplicates are okay since containers are shown separately).
      notes = document.querySelectorAll('.study-note');
      ...
      // When searching, only search in the All Studies container to avoid duplicates.
      notes = document.querySelectorAll('#study-container .study-note');
      // Hide recommended container and its subheader.
      ...
      // Hide verse cubes and All Studies subheader.
  ...
  document.querySelectorAll('.verse-cube').forEach(cube => {
    cube.addEventListener('click', function() {
      collapseAllStudies(); // Collapse all open studies when a verse cube is pressed.
